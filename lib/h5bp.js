 ...
 *
 * @param {Boolean|Object|Function} [options.logger=false] true or false or logger options.
...
h5bp.createServer = function(options, callback) {
	...
	if (true === options.logger || /object|function/.test(typeof options.logger)) {
		app.use(express.logger(options.logger));
	}
	// then compress
	if (false !== options.compress) {
		var compress = options.compress;
		if ('object' != typeof compress) {
			compress = {};
		}
		// compress filter
		if (!compress.filter) {
			compress.filter = function(req, res) {
				return /^(?:text\/.*|application\/(?:j(?:son|avascript)|(?:(?:xhtml|rss|atom)\+)?xml))/i
					.test(res.getHeader('Content-Type'));
			};
		}
		app.use(express.compress(compress));
	}
	// h5bp right after
