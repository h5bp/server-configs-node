...
/** mime type regexps */
const RE_MIME_IMAGE = /image/;
const RE_MIME_FONT = /(application\/(font-woff|x-font-ttf|vnd.ms-fontobject|font\/opentype))/;
const RE_MIME_DATA= /(text\/(cache-manifest|html|xml)|application\/(xml|json))/;
const RE_MIME_FEED = /application\/(rss\+xml|atom\+xml)/;
const RE_MIME_FAVICON = /image\/x-icon/;
const RE_MIME_MEDIA = /(image|video|audio|text\/x-component|application\/(font-woff|x-font-ttf|vnd.ms-fontobject|font\/opentype))/;
const RE_MIME_CSSJS = /(text\/(css|x-component)|application\/javascript)/;

/** misc regexps */
const RE_WWW = /^www\./;
const RE_MSIE = /MSIE/;
const RE_HIDDEN = /(^|\/)\./;
const RE_SRCBAK = /\.(bak|config|sql|fla|psd|ini|log|sh|inc|swp|dist)|~/;

/** overrides default mime types */
    ...
    return function(req, res, next) {
        ...
        try {
            ...
            if (false === options.www && RE_WWW.test(host)) {
                redirect(res, '//' + host.replace(RE_WWW, '') + url);
            }
            ...
            else if (true === options.www && !RE_WWW.test(host)) {
                redirect(res, '//www.' + host.replace(RE_WWW, '') + url);
            }
