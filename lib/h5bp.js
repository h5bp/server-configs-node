...
h5bp = module.exports = function(options) {
	...
	if (options.stylesheets) {
		if (!options.stylesheets.processor) {
			options.stylesheets.processor = 'less';
		}
		else if (!/less|sass/.test(options.stylesheets.processor)) {
			throw new Error("Stylesheet compilation processor can be either 'less' or 'sass'");
		}

		if ('string' == typeof options.stylesheets.files) {
			options.stylesheets.files = [options.stylesheets.files];
		}
		if (!Array.isArray(options.stylesheets.files)) {
			throw new Error("There is no stylesheet to process");
		}

		layers.push(require('./layers/' + options.stylesheets.processor)(options));
	}
